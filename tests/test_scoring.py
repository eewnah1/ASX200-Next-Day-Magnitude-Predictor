...
def test_high_vol_increases_high_bucket(engine: ScoringEngine):
    low = engine.predict(
        FeatureVector(a_vix=10.0, realized_vol_annual=10.0)
    ).probabilities.high
    high = engine.predict(
        FeatureVector(a_vix=35.0, realized_vol_annual=35.0)
    ).probabilities.high
    assert high > low
...
def test_trend_session_increases_high_bucket(engine: ScoringEngine):
    low = engine.predict(
        FeatureVector(asx_session_character="range")
    ).probabilities.high
    high = engine.predict(
        FeatureVector(asx_session_character="trend")
    ).probabilities.high
    assert high > low
...
def test_alignment_magnitude_increases_high_bucket(engine: ScoringEngine):
    low = engine.predict(
        FeatureVector(cross_asset_magnitude=0.0)
    ).probabilities.high
    high = engine.predict(
        FeatureVector(cross_asset_magnitude=3.0)
    ).probabilities.high
    assert high > low
