...
MCP servers expose quotes, history, filings and indicators as tools for
Claude/Cursor.  These dashboards need the *same data* inside the prediction
loop, so this module talks to the underlying free HTTP feeds that those MCP
servers wrap — no extra process, no paid key required.

Stack (always-on, fail-soft):
  - Yahoo Finance chart v8  (FinMCP / yfinance / ASX_MCP class coverage)
  - Local quant toolkit     (returns, RSI, vol regime, factor tilt)
  - Optional remote MCP HTTP probes (StockMCP, Equibles) when reachable
  - Optional Alpha Vantage REST if ALPHAVANTAGE_API_KEY is set

Never raises.  Never blocks the core predictor for more than a few seconds.
"""
...
DEFAULT_UNIVERSES: dict[str, dict[str, str]] = {
    "asx200": {
        "XJO": "^AXJO", "SPI": "AP=F", "CBA": "CBA.AX", "BHP": "BHP.AX",
        "RIO": "RIO.AX", "NAB": "NAB.AX", "WBC": "WBC.AX", "ANZ": "ANZ.AX",
        "AUDUSD": "AUDUSD=X", "IRON": "TIO=F", "GOLD": "GC=F",
        "SPX": "^GSPC", "NDX": "^IXIC", "VIX": "^VIX",
    },
    "ausuper": {
        "XJO": "^AXJO", "SPX": "^GSPC", "NDX": "^IXIC", "ACWI": "ACWI",
        "EEM": "EEM", "AGG": "AGG", "BND": "BND", "GLD": "GLD",
        "AUDUSD": "AUDUSD=X", "VIX": "^VIX", "STW": "STW.AX", "IOZ": "IOZ.AX",
    },
    "international": {
        "ACWI": "ACWI", "VXUS": "VXUS", "EFA": "EFA", "EEM": "EEM",
        "SPX": "^GSPC", "NDX": "^IXIC", "NKY": "^N225", "HSI": "^HSI",
        "KS11": "^KS11", "STOXX": "^STOXX50E", "DXY": "DX-Y.NYB",
        "VIX": "^VIX", "USDJPY": "USDJPY=X",
    },
    "pinebridge": {
        "EEM": "EEM", "AAXJ": "AAXJ", "FXI": "FXI", "EWY": "EWY",
        "EWT": "EWT", "INDA": "INDA", "EWS": "EWS", "NKY": "^N225",
        "HSI": "^HSI", "KS11": "^KS11", "TWII": "^TWII", "SENSEX": "^BSESN",
        "USDJPY": "USDJPY=X", "USDCNY": "USDCNY=X", "VIX": "^VIX",
    },
    "ut_switch": {
        "STI": "^STI", "SPX": "^GSPC", "NDX": "^IXIC", "HSI": "^HSI",
        "NKY": "^N225", "XJO": "^AXJO", "EEM": "EEM", "GLD": "GLD",
        "TLT": "TLT", "USDSGD": "USDSGD=X", "VIX": "^VIX",
        "ES3": "ES3.SI", "G3B": "G3B.SI",
    },
}
...
def _yahoo_chart(ticker: str, range_: str = "3mo", interval: str = "1d", timeout: float = 8.0) -> dict[str, Any]:
    ...
    return {
        ...
        "ret_1d_pct": _round(ret_1d),
        "ret_5d_pct": _round(ret_5d),
        "ret_21d_pct": _round(ret_21d),
        "ann_vol_21d_pct": _round(vol),
        "rsi_14": _round(_rsi(closes, 14)),
        "as_of": datetime.fromtimestamp(ts[-1], tz=timezone.utc).isoformat() if ts else _now_iso(),
...
def _probe_remote_mcp(name: str, url: str, timeout: float = 3.0) -> dict[str, Any]:
    try:
        resp = requests.get(
            url,
            headers={"Accept": "application/json, text/event-stream", "User-Agent": YAHOO_UA},
            timeout=timeout,
...
def _factor_tilt(quotes: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "bias": bias,
        "score": _round(score),
        "confidence": _round(min(0.85, 0.35 + abs(score) / 4.0), 3),
        "risk_on_avg_1d_pct": _round(risk_on_avg),
        "vix_1d_pct": _round(vix),
        "names_used": [n for n, q in quotes.items() if q.get("ret_1d_pct") is not None],
    }
...
def fetch_free_mcp_stack(universe: str = "asx200", extra_tickers: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "quotes": quotes,
        "ok_count": len(ok_quotes),
        "requested_count": len(tickers),
        "tilt": tilt,
        "remote_mcp_probes": remote,
        "alpha_vantage": av,
        "sources_ok": sources_ok,
        "errors": errors[:12],
        "note": "Yahoo chart is the free MCP data plane for US + APAC (.AX .SI .HK .T .KS). Remote MCP URLs are probed only.",
    }
...
def overlay_prediction(base: dict[str, Any], stack: dict[str, Any], *, key_field: str = "bias") -> dict[str, Any]:
    ...
    out["mcp_stack"] = {
        ...
        "tilt": tilt,
        "sources_ok": stack.get("sources_ok"),
        "ok_count": stack.get("ok_count"),
        "requested_count": stack.get("requested_count"),
...
if __name__ == "__main__":
    print(json.dumps(fetch_free_mcp_stack("asx200"), indent=2, default=str)[:2000])
